...
class InvariantPointAttention(nn.Module):
    ...
    def __init__(
        ...
        self.c_s = ipa_conf.c_s # node_embed_size
        self.c_z = ipa_conf.c_z # edge_embed_size
    ...
    def forward(
        ...
        #######################################
        # [*, N_res, H * C_hidden]
        q = self.linear_q(s) # one mlp get q
        kv = self.linear_kv(s) # one mlp get kv
        ...
        q_pts = r[..., None].apply(q_pts) # apply the transformation to the q_pts
        ...
        # [*, H, N_res, N_res]
        a = torch.matmul( # reorganize the last 3 dimensions
            permute_final_dims(q, (1, 0, 2)),  # [*, H, N_res, C_hidden]
            permute_final_dims(k, (1, 2, 0)),  # [*, H, C_hidden, N_res]
        )
        a *= math.sqrt(1.0 / (3 * self.c_hidden)) # scale by number of dimension
        a += (math.sqrt(1.0 / 3) * permute_final_dims(b, (2, 0, 1))) # add the bias
        ...
        # [*, N_res, N_res, H, P_q]
        pt_att = sum(torch.unbind(pt_att, dim=-1)) # torch.unbind return a tuple of slices in the last dimension
        head_weights = self.softplus(self.head_weights).view(
            *((1,) * len(pt_att.shape[:-2]) + (-1, 1)) # (*, 1, 1, no_heads, 1)
        )
        ...
        a = self.softmax(a) # softmax for the last dimension - for each residue, softmax its attention weight with other residue
        ...
        # [*, N_res, H, C_hidden]
        o = torch.matmul(
            a, v.transpose(-2, -3) # a: (*, H, N_res, N_res), v: (*, N_res, H, C_hidden)
        ).transpose(-2, -3)
