...
class EdgeFeatureNet(nn.Module):
    ...
    def __init__(self, module_cfg):
        ...
        self.c_s = self._cfg.c_s # node_embed_size
        self.c_p = self._cfg.c_p # edge_embed_size
        self.feat_dim = self._cfg.feat_dim # hidden_dim
    ...
    def forward(self, s, t, sc_t, p_mask, diffuse_mask):
        ...
        cross_node_feats = self._cross_concat(p_i, num_batch, num_res) # (b, n_res, n_res, 2*feat_dim)
        ...
        dist_feats = calc_distogram(
            t, min_bin=1e-3, max_bin=20.0, num_bins=self._cfg.num_bins) # (b, n_res, n_res, num_bins)
