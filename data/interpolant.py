...
class Interpolant:
    ...
    def __init__(self, cfg):
        self._cfg = cfg
        self._rots_cfg = cfg.rots # rotation config
        self._trans_cfg = cfg.trans # translation config
        self._sample_cfg = cfg.sampling # sampling config
        self._igso3 = None
    ...
    def sample_t(self, num_batch):
        ...
        return t * (1 - 2*self._cfg.min_t) + self._cfg.min_t # sample t from [min_t, 1-min_t]

    def _corrupt_trans(self, trans_1, t, res_mask, diffuse_mask):
        trans_nm_0 = _centered_gaussian(*res_mask.shape, self._device)
        trans_0 = trans_nm_0 * du.NM_TO_ANG_SCALE # increase the variance to be du.NM_TO_ANG_SCALE
    ...
    def _corrupt_rotmats(self, rotmats_1, t, res_mask, diffuse_mask):
        ...
        rotmats_0 = torch.einsum(
            "...ij,...jk->...ik", rotmats_1, noisy_rotmats) # perturb the rot_1 but still remains orientation property of rot_1
    ...
    def corrupt_batch(self, batch):
        ...
        return noisy_batch # with ['rotmats_t'], ['trans_t'], ['r3_t'], and ['so3_t']
    ...
    def sample(
        ...
        ts = torch.linspace(self._cfg.min_t, 1.0, num_timesteps) # len(ts) = num_timesteps | ts: [min_t, ..., 1.0]
        t_1 = ts[0] # min_t

        prot_traj = [(trans_0, rotmats_0)] # first element is pure noise
        clean_traj = []
        for i, t_2 in enumerate(ts[1:]): # iterate num_steps times
            if verbose: # and i % 1 == 0:
                print(f'{i=}, t={t_1.item():.2f}')
                print(torch.cuda.mem_get_info(trans_0.device), torch.cuda.memory_allocated(trans_0.device))
            # Run model.
            trans_t_1, rotmats_t_1 = prot_traj[-1]
            if self._trans_cfg.corrupt: # corrupted data as input
            ...
            pred_trans_1 = model_out['pred_trans']
            pred_rotmats_1 = model_out['pred_rotmats']
            clean_traj.append( # save the predicted clean translation and rotation
            ...
            if trans_potential is not None: # gradient based optimization at each step
                with torch.inference_mode(False):
                    grad_pred_trans_1 = pred_trans_1.clone().detach().requires_grad_(True) # remove the prior gradients and enable the gradients
                    pred_trans_potential = autograd.grad(outputs=trans_potential(grad_pred_trans_1), inputs=grad_pred_trans_1)[0] # the gradient of the potential function
        ...
        if self._rots_cfg.corrupt:
            ...
            if rotmats_1 is None:
                raise ValueError('Must provide rotmats_1 if not corrupting.')
            batch['rotmats_t'] = rotmats_1
        batch['t'] = torch.ones((num_batch, 1), device=self._device) * t_1 # now t1 is (1.)
        with torch.no_grad(): # refine the sample at the last step
            model_out = model(batch)
        pred_trans_1 = model_out['pred_trans']
        pred_rotmats_1 = model_out['pred_rotmats']
