...
# define a threshold or cutoff for the radius of gyration that varies with sequence length. 
...
class BaseDataset(Dataset):
    def __init__(
        ...
        self._dataset_cfg = dataset_cfg # scope_dataset or pdb_dataset
        self.task = task
        self.raw_csv = pd.read_csv(self.dataset_cfg.csv_path) # './metadata/scope_metadata.csv' or './metadata/pdb_metadata.csv'
    ...
    def _create_split(self, data_csv):
        ...
        if self.is_training: # yes for training
            self.csv = data_csv
            self._log.info(
            ...
            eval_csv = data_csv[data_csv.modeled_seq_len.isin(eval_lengths)] # take only the data that is among the seq lens
